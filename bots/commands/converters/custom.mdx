...
Before getting started, make sure you set your project up [with our Gradle plugin](#),
or that you have [KSP](https://kotlinlang.org/docs/ksp-overview.html) version <EmbeddedVersion reference="ksp" code />
configured with the Kord Extensions annotation processor,
<code>dev.kordex:annotation-processor:<EmbeddedVersion reference="kordEx" /></code>.

For more information on how you can do this,
please read [the official KSP quickstart page](https://kotlinlang.org/docs/ksp-quickstart.html).

### Source Directories

If you're not [using our Gradle plugin](#), you may need to configure the KSP output directories as source roots.
When using Gradle with Kotlin, you can do that as follows:
...
Your converter must implement parsing for both chat command and slash command arguments.
...
- Then, create the `parseOption` function, and note that the `option` argument must take an `OptionValue<*>` rather than
  ...
  - Then, parse, store, throw, and return similarly to your `parse` function.
