...
:::warning Moved to dedicated plugin!

As of Gradle plugin version 1.9.0, we've removed i18n class generation feature in favour of
[the new, more flexible dedicated i18n framework plugin](/i18n/plugin).

As the new framework's types are in a different package from those we shipped with Kord Extensions versions earlier
than `2.4.0-SNAPSHOT`, this new plugin won't work with older versions.
If you're still using an older version of Kord Extensions, stick with version `1.8.1` of this plugin, and read the
below section for more information.

:::

<details>
  <summary>**Information for plugin version `1.8.1` and older.**</summary>
